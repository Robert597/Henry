export const products = [
{
    _id: 1,
    image: ['/assets/zanzibar.jpeg', '/assets/zanzibar.jpeg'],
    name: 'Bed Zanzibar',
    description: 'Wooden Bed with Leather and Wood Head board comfortable for sleeping',
    price: 857.99,
    priceId: 'price_1OfWLJKJgU8JtmfpfeKKCI2v'
},
{_id: 2,
    image: ['/assets/table.jpeg', '/assets/table.jpeg'],
    name: 'Center Table',
    description: '',
    price: 2300.99,
    priceId: 'price_1OfWReKJgU8JtmfpMenwDSX2'
},
{
    _id: 3,
    image: ['/assets/octavia.png', '/assets/octavia.png'],
    name: 'Bed Octavia',
    price: 980.99,
    description: 'Wooden Bed comfortable for sleeping',
    priceId: 'price_1OfWWoKJgU8JtmfpZO3CqnoW'
},
{
    _id: 4,
    image: ['/assets/sofa.jpeg', '/assets/sofa.jpeg'],
    name: 'Sofa',
    price: 4500,
    description: 'L-SHAPE FABRIC SOFA,ANIMAL SKIN, Length: 350 cm, Long side: 180 cm, Short side: 100 cm',
    priceId: 'price_1OfWb9KJgU8JtmfpswmORwQZ'
},
{
    _id: 5,
    image: ['/assets/stand.jpeg', '/assets/stand.jpeg'],
    name: 'Luxury Tv Stand',
    price: 5750.99,
    description: 'This Luxury Foreign tv stand will give your home a luxurious and elegant look. It has the capacity to support up to 20 kg of objects and devices, and it’s built with the best quality luxurious materials. When it comes to affordable luxury furnitures, VAVA Furniture have been serving the world with top-notch products with over 20 showrooms across the nation and several awards of excellence and quality production. We are dedicated to giving your home & offices a luxury look.',
    priceId: 'price_1OfWeEKJgU8JtmfpCVHFMXq2'
},
{
    _id: 6,
    image: ['/assets/dining.jpeg', '/assets/dining.jpeg'],
    name: 'Dining Table',
    price: 1800.99,
    description: '',
    priceId: 'price_1OfWlvKJgU8JtmfpLZMmLRJF'
},
{_id: 7,
    image: ['/assets/wine.jpg', '/assets/wine.jpg'],
    name: 'High quality wine cabinet',
    price: 1067,
    description: '',
    priceId: 'price_1OfWojKJgU8Jtmfp7JYBpsIo'
},
{_id: 8,
    image: ['/assets/DOOR.png', '/assets/DOOR.png'],
    name: 'High Quality door',
    price: 1067,
    description: 'sizes: 2100*900*200mm and 2100*750*200mm. Our premium indoor door, is a fusion of style, craftsmanship, and functionality that will redefine the way you experience your living spaces.',
    priceId: 'price_1OfWojKJgU8Jtmfp7JYBpsIo'
},
{_id: 9,
    image: ['/assets/conference.jpg', '/assets/conference.jpg'],
    name: 'CONFERENCE TABLE ',
    price: 4500,
    description: 'IMPORTED CONFERENCE, 8/ 2.4M',
    priceId: 'price_1OfXZhKJgU8JtmfpaBzOnZq7'
}
]